...
def _parse_project_configs(raw: str) -> list[JiraProjectConfig]:
    ...
    선택 필드: jira_custom_fields (기본 {}), statuses (기본 []), status_mapping (기본 {})
    """
    try:
        data = json.loads(raw)
    ...
    for i, item in enumerate(data):
        ...
        status_mapping_raw = item.get("status_mapping", {})
        if not isinstance(status_mapping_raw, dict):
            raise RuntimeError(
                f"JIRA_PROJECT_CONFIGS[{i}].status_mapping는 객체여야 합니다."
            )
        for mk, mv in status_mapping_raw.items():
            if not isinstance(mv, list):
                raise RuntimeError(
                    f"JIRA_PROJECT_CONFIGS[{i}].status_mapping.{mk}는 배열이어야 합니다."
                )
        ...
        configs.append(JiraProjectConfig(
            ...
            status_mapping={str(k): [str(v) for v in vs] for k, vs in status_mapping_raw.items()},
        ))
