...
| 채팅에서 일부 도구만 사용됨 | MCP Tool 조회 수 제한 | 아래 [채팅 환경 Tool 제한](#채팅-환경-tool-제한) 참조 |

#### 채팅 환경 Tool 제한

Claude 채팅(claude.ai)에서 MCP 서버를 사용할 때, **MCP Tool 조회 수에 limit 설정**이 적용되어 있어 등록된 전체 도구 중 **상위 N개만 조회 및 사용**될 수 있습니다.

이 경우 일부 도구가 채팅에서 인식되지 않거나 호출되지 않을 수 있으므로, 채팅 환경의 MCP 설정에서 **Tool limit 값을 충분히 높게 설정**해야 합니다.

> **참고:** Claude Code(CLI)에서는 이 제한이 적용되지 않으며, 모든 도구가 정상적으로 사용됩니다.
